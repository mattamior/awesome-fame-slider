...
const standard = (surname) => [
  `Delayed ${surname}`, `Jailed ${surname}`, `${surname}`, `Saint ${surname}`, `God ${surname}`, `Ancestor ${surname}`,
];
...
async function fetchImagePngData(url) {
  const response = await fetch(url, { headers:{ 'user-agent':'awesome-fame-slider-share-card-builder/1.0' } });
  if (!response.ok) throw new Error(`Could not fetch meme asset ${url}: HTTP ${response.status}`);
  return (await sharp(Buffer.from(await response.arrayBuffer())).png().toBuffer()).toString('base64');
}
const avatarData = (await readFile(path.resolve('public/avatars.svg'))).toString('base64');
const rankImageData = new Map();
for (const person of people) if (person.rankImageUrls) rankImageData.set(person.id, await Promise.all(person.rankImageUrls.map(fetchImagePngData)));
