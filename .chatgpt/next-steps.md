...
1. Complete Mark Zuckerberg next: source or assemble a recognizable six-rank internet meme/image pack, wire fixed roster / community SUBJECT / explicit-selection large art, regenerate his six X cards, pass CI/Deploy, then RPM checkpoint.
2. Continue in this order, checkpointing after each production deployment: Sam Altman -> Dario Amodei -> Demis Hassabis.
3. For every completed person, keep the same state semantics already established by Liang/Tibo/Musk/Huang: fixed default roster image; SUBJECT follows community leader; refresh defaults large art to community leader; explicit user selection takes over large art; slider uses a mechanical pointer; X shares preserve the user's selected rank image.
